...
- [Leave comments](#commentsb)
- [Use informative names](#namingb)
- [Format your code](#formatCodeb)
- [Be explicit](#explicitb)
...
See also [Section \@ref(orgCode)](#orgCodeb)
...
### Minimize Package Usage {#minPackagesb}
...
```{r staleCheckb, echo=TRUE, eval=FALSE}
...
### Minimize External Files {#exFilesb}
...
<!-- end list -->
``` {r cssCall1b, echo = TRUE, eval = FALSE}
dashboardPage(
  skin = "blue",
  dashboardHeader(
    title = "title",
...
See also [Section \@ref(css)](#cssb).


## Organizing Your Code {#orgCodeb}
...
4.  UI definition (see [Chapter \@ref(layout)](#layoutb))
5.  Server definition
6.  `boastApp` call
...
2.  App Metadata (see below)
3.  Any additional source files and custom functions
4.  UI definition (see [Chapter \@ref(layoutb)](#layoutb))
...
```{r listExample1b, echo=TRUE, eval=FALSE}
tags$ul(
...
1. All apps need to have the appropriate reference to the CSS file (See [Section \@ref(textStyle)](#textStyleb)).  
...
```{r classTag1b, echo=TRUE, eval=FALSE}
# HTML Tag Example 1
p(
  class = "hangingindent",
...
```{r metadatab, echo = TRUE, eval = FALSE}
