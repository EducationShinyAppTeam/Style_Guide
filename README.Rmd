...
title: "Style Guide"
author:
  - Neil Hatfield (njh5464@psu.edu)
  - \, Robert Carey (rpc5102@psu.edu)
...
This guide spells out the styling that should be used for all apps included in [BOAST](https://github.com/EducationShinyAppTeam/BOAST). Styling refers to several different aspects including:
...
To get started, clone the [Sample_App](https://github.com/EducationShinyAppTeam/Sample_App) template repo found on GitHub. This will provide you with a skeleton for how to organize your files as well as your code.
...
The [boastUtils](https://github.com/EducationShinyAppTeam/boastUtils) package was created to automate much of the design and development process. This will not only reduce the amount of work you'll need to do, it'll also make apps more consistent. We **strongly** recommend that you make use of this tool.
...
You should adhere to the [tidyverse style guide](https://style.tidyverse.org/), however, there are some additional practices you need to follow:
...
3. Format your code
   ...
   When passing values to parameters in a function, be explicit and include the parameter name in your code. For example,
     ...
     ```
4. Be aware of HTML Tags and how to use them correctly -- __Accessibility Issue__  
...
For the most part, any mathematics you need displayed should be done using [MathJax](https://www.mathjax.org/). Default to using inline typesetting with the `\\(` and `\\)` delimiters. If you need to use display style, you can use `\\[` and `\\]` or double dollar signs. For the vast majority of mathematics, you'll wrap both inline and display style mathematics inside of a paragraph environment.
...
If you are using the `boast.CSS` file, the text that is included in/on buttons, dropdown menus, sliders, radio buttons, choices, and other inputs as well as alert messages and popups/rollovers, will automatically be styled correctly. 
...
Unfortunately, text in `R` plots do not get controlled by CSS. This means that you'll have to play around with the settings.  Using the `ggplot2` package to make your plots (or other packages based upon the ggplot framework) will allow you to use the `theme` aspect to control text in your App.
...
You will need to play around with the settings to find the appropriate value; text size 18 does appear to work out well.
...
1. Overview--icon: Dashboard--__REQUIRED__
...
- Consider adding a loading bar to show the process of large number calculations.
...
**DO NOT sacrifice clarity and precision/accuracy for conciseness/brevity.**

Since these Apps are for *teaching*, we need to use language that is accurate and supports students in constructing productive meanings. This means that we need to avoid sloppy language and re-enforcing problematic conceptions. For example,
