...
4.  UI definition (see [Chapter \@ref(layout)](#layout))
5.  Server definition
6.  `boastApp` call
...
+ Both `APP_TITLE` and `APP_DESCP` do not follow camelCase. This is by intent to denote global `constants`.
+ The use of `<<-` rather than `<-`. This is again to deal with a global aspect.
