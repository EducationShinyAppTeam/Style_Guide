...
+ the text you want to be displayed. For accessibility, make the text descriptive of where you're sending the user.
...
{
  p(
    "Here's an example of using a horizontal rule to separate the
    above from the next section."
  )
  hr()
}
```
...
There are a few other tags which you might need to use, depending upon your app. These include the image tag, `img()`, the figure caption tag, `figcaption()`. We will discuss these tags in the chapter on [Static Images](#staticImages).
...
The key way that students can style elements is through the use of classes which live in the BOAST CSS file. A class refers a group of elements which should all have similar styling. The any CSS file essentially consists of class definitions. If you want to apply a particular styling to an app element, the preferred method is through a CSS class. 
...
You might run into cases where you want an element styled in a new way or you might be the first person to use some new element (e.g., Fall 2020 was the first time someone used `matrixInput`) that needs BOAST styling. We can create new classes to handle cases. However, we ask that you follow this process:

1. Get as many other bugs fixed in your app first.
2. Identify the element(s) that need the styling.
3. At least one idea for what you're wanting.
...
Be prepared during the meeting for Neil/Bob to ask lots of questions about the intent of the styling. We're doing this to make sure that the style decisions support the intent. By providing us with the most up-to-date version of your app, we can demo changes in real time for you. Once we get a class figured out, we'll add it to the BOAST CSS file and give you instructions on how to apply the class to the specific element(s).
