...
\hypertarget{using-rlocker}{%
\section{Using RLocker}\label{using-rlocker}}

{[}\emph{This section is under development.}{]}
...
The last element of the Sidebar will be the Penn State Logo. Please refer to Section \ref{logo}.
...
The tabs inside the body should automatically appear horizontally and along the top of the tab body (i.e., in the white space below the Dashboard Header). Any visual styling will be managed at the global level.
...
Buttons are one way in which users interact with the apps. The two most common functions used here are \texttt{shiny::actionButton} and \texttt{shinyBS::bsButton}. Both functions share many of the same features. Two ways in which they are different is that \texttt{shinyBS::bsButton} has an additional \texttt{style} argument while \texttt{shiny::actionButton} has a \texttt{width} argument that gives you fine grain size control (\texttt{bsButton} has a size option). There are three key styling aspects: shape/animation, color, and text \& icon.
...
Here are some guidelines for a button's text:
...
If you have more groups than eight/nine colors listed in two palettes, consider reworking your examples as you could overwhelm the user with too many colors (see Section \ref{graphics}). This admonishment also applies to line styles and point shapes.
...
Again, let the central CSS file handle the styling of this type of text by wrapping the text in the appropriate tags (typically \texttt{p()}).
...
All apps will need a References Tab. This is where you'll place all references for your App, including R packages, borrowed code, data sources, images, etc. This in addition to the Acknowledgements.

\textbf{NOTE:} listing something in the Acknowledgements DOES NOT waive this requirement.
